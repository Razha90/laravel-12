...
<div class="mx-auto mt-[12vh] h-[88vh] w-[90%] max-w-[1600px] overflow-hidden rounded-xl bg-white">
    <div class="h-full w-full bg-white">
        <center class="m-auto mt-24">
            ...
            <div class="mt-4 tracking-widest">
                @if (session()->has('error'))
                    <span class="block text-6xl text-gray-500"><span>{{ session('code') }}</span></span>
                    <span class="text-xl text-gray-500">{{ session('error') }}</span>
                @else
                    <span class="block text-6xl text-gray-500"><span>5 0 0</span></span>
                    <span class="text-xl text-gray-500">{{ __('error.server_error') }}</span>
                @endif

            </div>
        </center>
        <center class="mt-6">
            <a href="/" wire:navigate class="rounded-md bg-gray-200 p-3 font-mono text-xl text-gray-500 hover:shadow-md">Go back
            </a>
        </center>
    </div>
</div>
